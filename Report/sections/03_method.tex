...
Note that \texttt{fw-medium} (16 layers) is significantly deeper than the paper's \texttt{ts-medium} (6 layers).
While this pivot was necessary for SAE coverage, we acknowledge that deeper architectures generally exhibit more complex feature interactions, potentially making the low-rank approximation task more difficult than in the original setting.
...
To enable reproducibility on consumer-grade hardware (e.g., single GPU), we implemented a memory-efficient streaming algorithm for the eigendecomposition of the interaction tensor $\mathcal{I}$, avoiding the instantiation of the full $d_{\text{in}} \times d_{\text{in}} \times d_{\text{out}}$ tensor.
