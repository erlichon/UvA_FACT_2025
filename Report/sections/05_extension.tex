...
\subsubsection{Motivation: From Post-Hoc to Intrinsic Interpretability}
...
\end{equation}
where $R$ is the maximum rank, and $\lambda_r$ are learnable scaling factors. Crucially, this factorization does not enforce orthogonality. This flexibility allows for \emph{disentangled} representations where factors can naturally overlap, aligning more closely with human intuition of how visual patterns decompose into atomic parts (e.g., strokes, loops).

\subsubsection{Methodology: Regularization Variants}
...
\begin{itemize}
    ...
    \item \textbf{Lambda CP (Magnitude Regularization):} We introduce learnable scalar scales $\lambda_r$ for each rank component, subject to $L_1$ regularization. This encourages ``soft sparsity,'' allowing the model to downweight unnecessary ranks while retaining the capacity to utilize them if necessary.
    
    \item \textbf{Gated CP (Sparsity Regularization):} Inspired by probabilistic masking, we apply stochastic gates $z_r \sim \text{Bernoulli}(\sigma(\phi_r))$ to each rank component, where $\phi_r$ are learnable parameters. We optimize an $L_0$ proxy objective ($\sum_r \sigma(\phi_r)$) to explicitly prune unnecessary ranks. This method allows the model to self-select the minimal set of active ranks required for the task.
\end{itemize}
...
\subsubsection{Quantitative Analysis: The Accuracy-Interpretability Trade-off}
...
\subsubsection{Qualitative Analysis: Disentangled Factors}
...
CP decomposition fundamentally alters this scaling law to $O(R \cdot (d_{\text{in}} + d_{\text{out}}))$, where the rank $R$ is a user-controlled parameter often much smaller than $d$.
