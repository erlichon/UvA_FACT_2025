...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=2pt]
    ...
    \item \textbf{Language Reproduction (Section~\ref{sec:language_results}):} Partially reproduced. We confirm AND-gate negation circuits with semantic opposites (not-good vs.\ not-bad), but the fraction of features achieving high ($>$0.75) rank-2 correlation is lower than suggested by the paper.
    ...
    \item \textbf{Extension 2 (Section~\ref{sec:cp_extension}):} We explore CP-decomposition as an architectural constraint for intrinsic low-rank structure, achieving 93.8\% accuracy with disentangled, atomic feature representations, while gaining significant efficiency (23 times faster training).
\end{itemize}
