...
def plot_cosine_similarity_heatmap(
    ...
    """
    Plot cosine similarity heatmap between top-k eigenvectors of digit and letter.
    
    Shows pairwise cosine similarities to reveal if eigenvector #1 of digit
    matches eigenvector #1 of letter (diagonal should be high if aligned).
    
    Args:
        digit_vecs: [n_components, 784] eigenvectors for one MNIST digit
        letter_vecs: [n_components, 784] eigenvectors for one EMNIST letter
        digit_vals: [n_components] eigenvalues for sorting
        letter_vals: [n_components] eigenvalues for sorting
    ...
    _, digit_sorted_idx = digit_vals.abs().sort(descending=True)
    _, letter_sorted_idx = letter_vals.abs().sort(descending=True)
    
    digit_top_k = digit_vecs[digit_sorted_idx[:k]].cpu()  # [k, 784]
    letter_top_k = letter_vecs[letter_sorted_idx[:k]].cpu()  # [k, 784]
    
    # Normalize eigenvectors
    digit_top_k = digit_top_k / (digit_top_k.norm(dim=1, keepdim=True) + 1e-10)
    letter_top_k = letter_top_k / (letter_top_k.norm(dim=1, keepdim=True) + 1e-10)
    
    # Compute cosine similarity matrix
    cos_sim = (digit_top_k @ letter_top_k.T).numpy()  # [k, k]
    ...
    ax.set_xticklabels([f'L{i+1}' for i in range(k)])
    ax.set_yticklabels([f'D{i+1}' for i in range(k)])
    ax.set_xlabel(f"EMNIST '{letter_label}' Eigenvector Index", fontsize=11)
    ax.set_ylabel(f"MNIST '{digit_label}' Eigenvector Index", fontsize=11)
    ax.set_title(f"Cosine Similarity: '{digit_label}' vs '{letter_label}' Top-{k} Eigenvectors", fontsize=12)
